...
import ticking from "../../sounds/tick-tock.wav";
...
const PomodoroTimer = () => {
  //useContext
  const { currentTask, updateCompletedPomosNo, updatePomosNo } = useContext(
    TaskContext
  ) as TaskContextType;
  const {
    pomodoro,
  ...
  const { playAlarm, alarmSound, playTicking, volume } = useContext(
    AlarmContext
  ) as AlarmContextType;
  const { pomodoroMode, timerMode, updatePomodoroMode, updateTimerMode } =
    useContext(TimerModesContext) as TimerModesContextType;
  ...
  const [alarmSoundSrc, setAalrmSoundSrc] = useState(chime);
  const [alarmPlayStatus, setAlarmPlayStatus] = useState(PlayStatus.stopped);
  const [tickingPlayStatus, setTickingPlayStatus] = useState(
    PlayStatus.stopped
  );
  ...
  useEffect(() => {
    // alarmSound if changes
    switch (alarmSound) {
      case AlarmSounds.chime: {
        setAalrmSoundSrc(chime);
        break;
      }
      case AlarmSounds.bell: {
        setAalrmSoundSrc(bell);
        break;
      }
    }
    ...
    if (timerMode === TimerModes.play) {
      intervalId.current = setInterval(() => {
        if (playTicking) {
          setTickingPlayStatus(PlayStatus.playing);
        }
        if (time.seconds === 0) {
          if (time.minutes === 0) {
            switchBetweenBreaksAndPomos();
            setTickingPlayStatus(PlayStatus.stopped);
  ...
  return (
    <section className="timer-container">
      <div className={`container center ${pomodoroMode}`}>
        <div className="timer">
          <div className="timer-title center">{timerTitle}</div>
          <div className="timer-title center">#{pomoCount}</div>
      ...
      <TimerButtons
        pauseOnClick={handleTimerPause}
        playOnClick={handleTimerStart}
        skipOnClick={handleTimerSkip}
      />
      ...
      <AlarmSound
        url={alarmSoundSrc}
        playStatus={playAlarm ? alarmPlayStatus : PlayStatus.stopped}
        volume={volume}
        onFinishedPlaying={() => setAlarmPlayStatus(PlayStatus.stopped)}
      />
      <AlarmSound
        url={ticking}
        playStatus={tickingPlayStatus}
        volume={volume}
      />
    </section>
  );
