...
const Settings = (props: Props) => {
  ...
  const { playAlarm, alarmSound, playTicking, volume, updateAlarmSettings } =
    useContext(AlarmContext) as AlarmContextType;
  ...
  const [longBreakIntervalNum, setLongBreakIntervalNum] =
  ...
  const handleShortBreakTimeOnChange = (
  ...
  const handleLongBreakTimeOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
    if (!isFloat(event.currentTarget.value)) return;
  ...
  const handleAutoStartBreaksOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
    setAutoStartBreaksFlg(!autoStartBreaksFlg);
  };

  const handleAutoStartPomosOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
    setAutoStartPomosFlg(!autoStartPomosFlg);
  };

  const handleLongBreakIntervalOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
  ...
  const handlePlayAlarmSoundOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
  ...
  const handlePlayTickingSoundOnChange = (
    event: React.FormEvent<HTMLInputElement>
  ) => {
    setPlayTickingSound(!playTickingSound);
  };

  const handleVolumeLvlOnChange = (
    event: Event,
    value: number | number[],
    activeThumb: number
  ) => {
  ...
  const setts = (
    <form className="settings-form" onSubmit={handleSaveButtonOnClick}>
      <div className="settings-header align-inline">
        <h3
          className={`settings-tab ${
            openTab === Tabs.settings ? "active" : ""
          }`}
          onClick={() => setOpenTab(Tabs.settings)}
        >
          <IoIosTimer /> Timer
        </h3>
        <h3
          className={`settings-tab ${openTab === Tabs.alarm ? "active" : ""}`}
          onClick={() => setOpenTab(Tabs.alarm)}
        >
          <IoIosAlarm /> Alarm
        </h3>
        ...
        <div className="settings-input">
          ...
          <div className="input-line padding-top-bottom">
            ...
            <input
              ...
              min="0"
              value={longBreakIntervalNum}
              onChange={handleLongBreakIntervalOnChange}
            />
          </div>
        </div>
      ) : (
      ...
      <div className="settings-footer">
        <input className="modal-btn right-side" type="submit" value="Save" />
      </div>
    </form>
  );

  return <Modal children={setts} isOpen={props.isOpen} toggle={props.toggle} />;
};
